...
func PatchResource(api huma.API, path *huma.PathItem) {
	...
	adapter.Handle(op, func(ctx huma.Context) {
		...
		ressourcePath := findRelativeRessourcePath(ctx.URL().Path, put.Path)

		// Perform the get!
		origReq, err := http.NewRequest(http.MethodGet, ressourcePath, nil)
		if err != nil {
		...
		putReq, err := http.NewRequest(http.MethodPut, ressourcePath, bytes.NewReader(patched))
		if err != nil {
