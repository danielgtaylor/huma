...
var validateTests = []struct {
	...
	},
	{
		...
		typ:   reflect.TypeFor[map[any]int](),
...
func TestValidateCustomFormatter(t *testing.T) {
	...
	registry := huma.NewMapRegistry("#/components/schemas/", huma.DefaultSchemaNamer)
	s := registry.Schema(reflect.TypeFor[struct {
		Value string "json:\"value\" format:\"email\""
	}](), true, "TestInput")
	pb := huma.NewPathBuffer([]byte(""), 0)
	res := &huma.ValidateResult{}
...
func TestValidateSchemaTransformerDeleteField(t *testing.T) {
	registry := huma.NewMapRegistry("#/components/schemas/", huma.DefaultSchemaNamer)
	s := registry.Schema(reflect.TypeFor[*TransformDeleteField](), true, "TestInput")
	pb := huma.NewPathBuffer([]byte(""), 0)
	res := &huma.ValidateResult{}
...
func Test_validateWithDiscriminator(t *testing.T) {
	registry := huma.NewMapRegistry("#/components/schemas/", huma.DefaultSchemaNamer)
	catSchema := registry.Schema(reflect.TypeFor[Cat](), true, "Cat")
	dogSchema := registry.Schema(reflect.TypeFor[Dog](), true, "Dog")
